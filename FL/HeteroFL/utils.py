...
def resume(model, model_tag, optimizer=None, scheduler=None, load_tag='checkpoint', strict=True, verbose=True):
    if cfg['data_split_mode'] != 'none':
        # Search for all files ending with _best.pt
        file_pattern = f'./output/model/{model_tag}/*_best.pt'
        matching_files = glob(file_pattern)

        # Select the latest matching file by modification time
        if matching_files:
            latest_file = max(matching_files, key=os.path.getmtime)
            checkpoint = load(latest_file)
            ...
            if verbose:
                print(f"Resumed from {latest_file} at epoch {last_epoch}")
        else:
            print(f"No file found with pattern '{file_pattern}'. Starting from scratch.")
