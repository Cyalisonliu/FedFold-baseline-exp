...
def runExperiment():
    ...
    n_split = int(cfg['data_split_mode'].split('-')[-1])
    train_ratio = []
    for m in cfg['model_mode'].split('-'):
        if m == 'only':
            continue
        train_ratio.append(int(cfg['model_split_rate'][m[0]]//cfg['model_split_rate']['e']))
    train_ratio = '-'.join([str(i) for i in train_ratio])
    fname = f"./log/{cfg['model_name']}_{cfg['data_name']}_non-iid-{n_split}_train-ratio-{train_ratio}_HeteroFL_{cfg['model_mode']}_{time_stamp}.txt"
    log_f = open(fname, 'w')
    ...
    for epoch in range(last_epoch, cfg['num_epochs']['global'] + 1):
        ...
        test(dataset['test'], data_split['test'], label_split, test_model, logger, epoch, log_f)
        ...
        save(save_result, './output/model/{tag}/{time}_checkpoint.pt'.format(tag=cfg['model_tag'], time=time_stamp))
        if cfg['pivot'] < logger.mean['test/{}'.format(cfg['pivot_metric'])]:
            cfg['pivot'] = logger.mean['test/{}'.format(cfg['pivot_metric'])]
            shutil.copy('./output/model/{tag}/{time}_checkpoint.pt'.format(tag=cfg['model_tag'], time=time_stamp),
                        './output/model/{tag}/{time}_best.pt'.format(tag=cfg['model_tag'], time=time_stamp))
        logger.reset()
    logger.safe(False)
    return
...
def train(dataset, data_split, label_split, federation, global_model, optimizer, logger, epoch):
    ...
    print(f"total bytes of global model: {total_bytes}")
...
def test(dataset, data_split, label_split, model, logger, epoch, log_f):
    with torch.no_grad():
        metric = Metric()
        model.train(False)
        ...
        for i, input in enumerate(data_loader):
            ...
            acc = (output['score'].argmax(dim=1) == input['label']).float().mean()
            test_acc.append(acc)
        ...
        # save validation result for plot
        print(f"{epoch} {test_acc}", file=log_f)
        log_f.flush()
    return
...
class Local:
    ...
    def train(self, local_parameters, lr, logger):
        ...
        for param in model.parameters():
            total_bytes += param.data.nelement() * param.data.element_size()
        print(f"total bytes of model: {total_bytes}")
