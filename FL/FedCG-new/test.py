...
model_name = 'Conv'
model_tag += '_FedCG'
# if cfg['dataset'] == 'CIFAR10' and model_fn == ResNet18:
#     model_tag += '_CIFAR10_ResNet'
#     model_name = 'ResNet'
#
# if cfg['dataset'] == 'CIFAR100' and model_fn == ResNet18:
#     model_tag += '_CIFAR100_ResNet'
#     model_name = 'ResNet'
...
if cfg['random']:
    model_path = f"./output/random/{model_tag}"
else:
...
for i, dir_name in enumerate(sorted(os.listdir(model_path), reverse=True)):
    ...
    if cfg['device_ratio'] in dir_name: #and i%3 == 0:
        print(i, dir_name)
        # dir_name = 'S1-W9_2024-10-01 18:01:13'
...
for dir_name in dir_names:
    ...
    # save testing result
    if cfg['random']:
        result_dir = f"./evaluation/{cfg['dataset']}_random"
    else:
        result_dir = f"./evaluation/{cfg['dataset']}"
    os.makedirs(result_dir, exist_ok=True)
    file_name = f"{result_dir}/{cfg['device_ratio']}_{model_tag}{'_weight' if cfg['weight'] else ''}.txt"
